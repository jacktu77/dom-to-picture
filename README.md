...
```shell
npm install --save dom-to-picture
```
...
```js
/* ES6 */
import * as domToPicture from 'dom-to-picture';
// or
import { toPng, toJpeg, toBlob, toPixelData, toSvgDataURL } from 'dom-to-picture';

/* ES5 */
var domToPicture = require('dom-to-picture');
```
...
```js
domToPicture.toPng(document.getElementById('my-node'))
  .then(function (dataUrl) {
...
```js
domToPicture.toBlob(document.getElementById('my-node'))
  .then(function (blob) {
...
```js
domToPicture.toJpeg(document.getElementById('my-node'), { quality: 0.95 })
  .then(function (dataUrl) {
...
domToPicture.toSvgDataURL(document.getElementById('my-node'), {filter: filter})
  .then(function (dataUrl) {
    /* do something */
  });
```
...
```js
domToPicture.toCanvas(document.getElementById('my-node'))
...
For bugs and feature requests, please [create an issue](https://github.com/jacktu77/dom-to-picture/issues/new).
